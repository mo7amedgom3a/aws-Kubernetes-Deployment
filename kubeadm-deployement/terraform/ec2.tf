...
resource "aws_instance" "control_plane" {
  ...
  subnet_id     = aws_subnet.private[0].id # Place in Private Subnet
  ...
  user_data = file("k8s-node-setup.sh") # read the file content and pass to user data
}
...
resource "aws_instance" "worker" {
  ...
  user_data = file("k8s-node-setup.sh")
}
